...
class DatabaseSyncService {
    ...
    async syncContacts(contacts: Contact[]) {
        try {
            const response = await api.post('/sync/contacts',
                { contacts },
    ...
    // 同步聊天窗口到数据库
    async syncChats(chats: Chat[]) {
        try {
            const response = await api.post('/sync/chats',
                { chats },
                { headers: this.getAuthHeaders() }
            );
            return response.data;
    ...
    // 同步消息到数据库
    async syncMessages(messages: Message[]) {
        try {
            const response = await api.post('/sync/messages',
                { messages },
                { headers: this.getAuthHeaders() }
            );
            return response.data;
    ...
    // 搜索消息
    async searchMessages(query: string, contactId?: string, limit = 100) {
        try {
    ...
    // 本地存储和数据库双重保存消息
    async saveMessage(contactWhatsappId: string, message: Omit<Message, 'contactWhatsappId'>) {
